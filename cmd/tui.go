...
func (m tuiModel) load() tea.Cmd {
	return func() tea.Msg {
		...
		ports, discoverErr := m.source.DiscoverPorts(context.Background())
		return tuiLoadedMsg{
			status:  status,
			tunnels: tunnels,
			ports:   ports,
...
func tuiHeader(status *statusPayload) string {
	...
	return tuiHeaderStyle.Render(fmt.Sprintf("Sealtun TUI  region=%s  namespace=%s  profile=%s  daemon=%s", region, namespace, profile, daemon))
}
...
func (m tuiModel) runSelectedTunnelOperation(operation string) (tea.Model, tea.Cmd) {
	...
	case "repair":
		return m.confirmCommandOperation(tunnelID, []string{"repair", tunnelID}, "Run conservative repair actions")
	...
	case "domain-plan", "domain-add", "domain-set", "share-create", "share-revoke", "share-rotate", "policy-rate", "resources-set":
...
func tuiOperationItems(tunnel listItem) []list.Item {
	...
	items := []list.Item{
		...
		tuiListItem{title: "Repair", desc: "Run conservative repair actions after confirmation", value: "repair"},
	...
	if tunnelprotocol.IsHTTP(tunnel.Protocol) || strings.TrimSpace(tunnel.Protocol) == "" {
		items = append(items,
			...
			tuiListItem{title: "Domain Set", desc: "Directly set a custom domain after DNS is ready", value: "domain-set"},
...
func tuiPromptSpec(operation string) (tuiPromptDefinition, bool) {
	...
	case "domain-set":
		return tuiPromptDefinition{title: "Domain Set", help: "Enter a custom domain whose CNAME already points to the Sealos host.", placeholder: "app.example.com"}, true
	case "share-create":
...
func tuiPromptCommandArgs(operation, tunnelID, input string) ([]string, error) {
	...
	case "domain-set":
		...
		return []string{"domain", "set", tunnelID, input}, nil
	case "share-create":
...
func nextTUIView(view tuiView) tuiView {
	switch view {
	case tuiViewTunnels:
