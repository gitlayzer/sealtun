...
- Declarative configuration: `sealtun.yaml`, `apply -f`, `diff -f`, multi-tunnel management, stable names, `ttl`, Pod resources, HTTPS access policies, SSH declarations, or generic TCP declarations. Read `references/declarative.md`.
...
| Manage many tunnels or stable config | `apply --dry-run`, `diff`, then real `apply` when requested | `list`, `inspect` |
...
5. Do not run `up`, `expose`, real `apply`, `share create/revoke`, `domain add/clear`, `stop`, `cleanup`, or `logout` unless explicitly requested. Prefer `apply --dry-run` and `diff` before real apply.
...
- `apply`: preview with `apply --dry-run` and `diff`; after real apply verify every tunnel with `list` and `inspect`.
...
- For declarative work, use `apply --dry-run` and `diff` before real `apply` when feasible.
