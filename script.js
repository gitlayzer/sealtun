...
const translations = {
  zh: {
    ...
    metaDescription: "Sealtun 是面向 Sealos Cloud 的本地服务公网入口，支持 HTTPS、SSH、TCP、Linux 透明 connect、init 引导、资源可见性、watch、doctor 自动修复、Dashboard 和 AI Skill。",
    skip: "跳到正文",
    "nav.capabilities": "能力",
    "nav.latest": "最新功能",
    "nav.skills": "Skills",
    ...
    "hero.lead": "HTTPS 预览、SSH/TCP 四层入口、Linux 透明访问集群 Service/Pod、访问策略、自定义域名证书、临时分享、init 引导、资源可见性、watch、doctor 自动修复和 Dashboard 工作台都已经接好。更重要的是，AI 可以通过 Skill 直接替你操作。",
    "hero.primary": "安装 Skill",
    "hero.secondary": "查看 CLI 用法",
    "trust.login": "OAuth2 登录 / region / profile",
    "trust.protocols": "HTTPS / SSH / TCP / Connect",
    "trust.policy": "Basic Auth / Bearer / rate limit",
    ...
    "statement.body": "Sealtun 使用 Sealos Cloud 的 Kubernetes、Service 与 Ingress 能力，把本地服务通过加密隧道转发出去，也能在 Linux 本机透明访问集群内 Service FQDN、Service ClusterIP 和 Pod IP。开发预览、第三方回调、SSH 调试、数据库临时访问、集群内联调和团队评审链接，都可以走同一套 CLI。",
    ...
    "latest.title": "最新版本已经覆盖透明集群访问、引导、资源可见性、自动修复和工作台。",
    ...
    "latest.share.kicker": "Connect",
    "latest.share.title": "Linux 本机可以直接访问集群 Service 和 Pod",
    "latest.share.body": "`sealtun connect --check` 先检查 RBAC 和系统能力，`sudo sealtun connect` 临时写入 hosts 与 iptables，让本机工具直接访问 Service FQDN、ClusterIP 和 Pod IP，无需 SOCKS/HTTP 代理配置。",
    "latest.policy.kicker": "Access policy",
    "latest.policy.title": "访问策略在代理层执行",
    "latest.policy.body": "Basic Auth、Bearer Token、临时 token、IP allowlist/denylist、rate limit 和访问审计都由 Sealtun proxy 校验，不依赖 Ingress annotation。",
    ...
    "skill.body": "通过一条 npx 命令把仓库里的 Skill 装进 AI 工作流。之后你可以直接描述目标，比如初始化推荐流程、暴露本地服务、访问集群内 Service/Pod、加认证、创建临时分享、绑定域名、打开 dashboard、查看 resources、watch 状态或跑 doctor。",
    ...
    "install.body": "安装路径保持短，运行路径保持可审计。首次使用可以先跑 init 获取推荐命令；需要本机访问集群内 TCP 服务时先跑 connect --check；团队配置写进 YAML 后再 dry-run、diff、apply，必要时再通过 dashboard 查看资源、事件和命令预览。",
    "tabs.skill": "AI Skill",
    "tabs.domain": "域名",
    "tabs.share": "集群访问",
    "tabs.dashboard": "工作台",
    ...
    "cta.title": "把公网入口、集群访问、访问策略、资源观测、诊断和配置回收放进同一个工具。",
    "cta.button": "查看源码",
  ...
  en: {
    ...
    metaDescription: "Sealtun exposes local services through Sealos Cloud with HTTPS, SSH, TCP, Linux transparent connect, guided init, resources, watch, doctor fixes, Dashboard, and AI Skill support.",
    ...
    "hero.lead": "HTTPS previews, SSH/TCP L4 entrances, Linux transparent access to cluster Services/Pods, access policies, custom-domain certificates, temporary shares, guided init, resources, watch, doctor fixes, and Dashboard workbench are wired together. More importantly, AI can operate Sealtun through the Skill.",
    ...
    "statement.body": "Sealtun uses Sealos Cloud Kubernetes, Service, and Ingress primitives to forward local services through encrypted tunnels, and on Linux it can transparently access in-cluster Service FQDNs, Service ClusterIPs, and Pod IPs. Dev previews, callbacks, SSH debugging, temporary database access, cluster debugging, and review links can all use the same CLI.",
    "protocol.eyebrow": "Public entrances",
    ...
    "latest.eyebrow": "v0.0.25 capabilities",
    "latest.title": "The latest release covers transparent cluster access, guided onboarding, resource visibility, safe fixes, and the workbench.",
    "latest.domain.kicker": "Domains",
    ...
    "latest.share.kicker": "Connect",
    "latest.share.title": "Linux clients can reach cluster Services and Pods directly",
    "latest.share.body": "`sealtun connect --check` verifies RBAC and host capabilities, then `sudo sealtun connect` temporarily writes hosts and iptables rules so local tools can reach Service FQDNs, ClusterIPs, and Pod IPs without SOCKS or HTTP proxy setup.",
    "latest.policy.kicker": "Access policy",
    "latest.policy.title": "Access policy runs at the proxy layer",
    "latest.policy.body": "Basic Auth, Bearer Token, temporary tokens, IP allowlist/denylist, rate limits, and access audit are enforced by the Sealtun proxy without relying on Ingress annotations.",
    "latest.workbench.kicker": "Workbench",
    ...
    "skill.body": "Install the repository Skill into your AI workflow with one npx command. Then describe the outcome: run guided init, expose localhost, access cluster Services/Pods, add auth, create a temporary share, bind a domain, open the dashboard, view resources, watch state, or run doctor.",
    ...
    "install.body": "Install paths stay short and operation stays auditable. First use can start with init recommendations; run connect --check before transparent cluster access; put team config in YAML, run dry-run, diff, and apply, then inspect resources, events, and command previews through the dashboard.",
    "tabs.skill": "AI Skill",
    "tabs.domain": "Domains",
    "tabs.share": "Connect",
    ...
    "cta.eyebrow": "Open source and Sealos native",
    "cta.title": "Keep public entrances, cluster access, access policy, resource visibility, diagnostics, and config recovery in one tool.",
