...
const translations = {
  zh: {
    ...
    metaDescription: "Sealtun 是面向 Sealos Cloud 的本地服务公网入口，支持 HTTPS、SSH、TCP、自定义域名、临时分享、Profile、Dashboard Resources、模板、apply/export 和 AI Skill。",
    skip: "跳到正文",
    "nav.capabilities": "能力",
    "nav.latest": "最新功能",
    "nav.skills": "Skills",
    ...
    "hero.lead": "HTTPS 预览、SSH/TCP 四层入口、访问策略、自定义域名证书、临时分享、Profile、Dashboard Resources、模板和 YAML 生命周期都已经接好。更重要的是，AI 可以通过 Skill 直接替你操作。",
    ...
    "trust.resources": "Live / Resources / Discover",
    ...
    "latest.title": "最新功能已经覆盖多账号、访问策略、工作台资源视图和 YAML 生命周期。",
    ...
    "latest.profile.title": "Profile 和 region 可以管理多套 Sealos 环境",
    "latest.profile.body": "`login --profile` 保存账号、region、workspace 和 kubeconfig，`profile use` 与 `region use` 可以在多环境之间明确切换。",
    ...
    "latest.diagnose.body": "`doctor <tunnel-id>`、`inspect --remote`、logs、events 和 metrics 会把本地 daemon、远端 Pod、Service、Ingress、证书和 TCP 计数串起来。",
    "latest.resources.kicker": "Live resources",
    "latest.resources.title": "Live、Resources 和 Discover 放在工作台里",
    "latest.resources.body": "Dashboard 会在 Live、Reconnecting、Polling 间切换；Resources 展示 Deployment、Pod、Service、Ingress、Certificate、Issuer 和 Secret 摘要，Discover 可以扫描本机监听端口并预填新隧道。",
    "skill.title": "不用先学 Sealtun，让 AI 先学会。",
    "skill.body": "通过一条 npx 命令把仓库里的 Skill 装进 AI 工作流。之后你可以直接描述目标，比如暴露本地服务、加认证、创建临时分享、绑定域名、打开 dashboard、导出 sealtun.yaml 或跑 doctor。",
    ...
    "install.body": "安装路径保持短，运行路径保持可审计。你可以临时用 npx，也可以全局安装 CLI；团队配置写进 YAML 后再 dry-run、diff、apply，必要时再通过 dashboard 查看资源和事件。",
    "tabs.skill": "AI Skill",
    "tabs.domain": "域名",
    "tabs.share": "分享",
    "tabs.dashboard": "工作台",
  ...
  en: {
    ...
    metaDescription: "Sealtun exposes local services through Sealos Cloud with HTTPS, SSH, TCP, custom domains, temporary shares, profiles, Dashboard Resources, templates, apply/export, and AI Skill support.",
    ...
    "hero.lead": "HTTPS previews, SSH/TCP L4 entrances, access policies, custom-domain certificates, temporary shares, profiles, Dashboard Resources, templates, and the YAML lifecycle are already wired together. More importantly, AI can operate Sealtun through the Skill.",
    ...
    "latest.eyebrow": "Latest capabilities",
    "latest.title": "The latest work covers multi-account state, access policy, workbench resources, and the YAML lifecycle.",
    "latest.domain.kicker": "Domains",
    ...
    "latest.profile.kicker": "Profiles",
    "latest.profile.title": "Profiles and regions manage multiple Sealos environments",
    "latest.profile.body": "`login --profile` saves account, region, workspace, and kubeconfig state. `profile use` and `region use` make environment switches explicit.",
    "latest.share.kicker": "Share",
    ...
    "latest.diagnose.body": "`doctor <tunnel-id>`, `inspect --remote`, logs, events, and metrics connect local daemon, remote Pod, Service, Ingress, certificate, and TCP counter checks.",
    "latest.resources.kicker": "Live resources",
    "latest.resources.title": "Live, Resources, and Discover live in the workbench",
    "latest.resources.body": "Dashboard switches between Live, Reconnecting, and Polling; Resources summarizes Deployment, Pod, Service, Ingress, Certificate, Issuer, and Secret metadata; Discover scans local listening ports and prefills new tunnels.",
    "skill.title": "You do not have to learn Sealtun first. Let AI learn it.",
    "skill.body": "Install the repository Skill into your AI workflow with one npx command. Then describe the outcome: expose localhost, add auth, create a temporary share, bind a domain, open the dashboard, export sealtun.yaml, or run doctor.",
    ...
    "install.body": "Install paths stay short and operation stays auditable. Use npx temporarily or install the CLI globally; put team config in YAML, run dry-run, diff, and apply, then inspect resources and events through the dashboard when needed.",
    "tabs.skill": "AI Skill",
    "tabs.domain": "Domains",
    "tabs.share": "Share",
